...
| 属性 | 触发 |
|------|------|
| `width` / `height` / `margin` / `padding` / `top` / `left` / `position` / `display` | **reflow** |
| `font-size` / `line-height` / `text-align` | **reflow**(影响文本布局) |
| `color` / `background` / `box-shadow` / `border-color` / `visibility` | **repaint** |
| `transform` / `opacity` / `filter` | **合成**(不重排不重绘) |

> 记法:改**几何/尺寸** → reflow;改**颜色/阴影** → repaint;改 `transform/opacity` → 仅合成。动画该选哪些属性,看这张表。
...
📎 参考:Google Web Fundamentals《避免大型复杂布局与布局抖动》、[MDN《Reflow》](https://developer.mozilla.org/zh-CN/docs/Glossary/Reflow)
