...
## 二、为什么 transform/opacity 能绕过主线程
...
**深层:层提升为什么经常伴随"新的层叠上下文/包含块"?** 一个元素被提升为合成层,常需**独立的绘制与定位基准**(重算包含块,见 [3.2](/卷三-HTML与CSS/3.2-盒模型与包含块)),所以 `transform`/`filter` 会"劫持"后代的包含块——这就是 3.2 里"transform 偷走 fixed 包含块"的底层原因。
...
transform 会让元素**成为合成层并建立新的包含块**(见 [3.2](/卷三-HTML与CSS/3.2-盒模型与包含块)),内部 `fixed` 后代改以它为参照,不再相对视口,于是"跑偏"。这是层提升与包含块机制的联动。
...
📎 权威延伸阅读:
- 《Animations: Compositing》(web.dev) https://web.dev/articles/animations-guide
- 《Rendering performance》(web.dev) https://web.dev/articles/rendering-performance
- 《GPU accelerated compositing in Chrome》(Chromium) https://www.chromium.org/developers/design-documents/gpu-accelerated-compositing-in-chrome/