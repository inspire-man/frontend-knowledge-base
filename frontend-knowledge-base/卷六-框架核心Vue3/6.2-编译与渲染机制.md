...
| 2 | CLASS | 动态 class |
| 4 | STYLE | 动态 style |
| 8 | PROPS | 动态属性(不含 class/style) |
| 16 | FULL_PROPS | 含 key 的动态属性 |
| 64 | STABLE_FRAGMENT | 顺序稳定的片段 |
| 128 | KEYED_FRAGMENT | 带 key 的片段 |

```js
// 位运算判断:patch 时只处理"有对应位"的动态点
if (vnode.patchFlag & 1)  { /* 只 patch 文本 */ }
if (vnode.patchFlag & 2)  { /* 只 patch class */ }
```

> 一个节点同时有动态文本 + 动态 class,其 patchFlag = `1 | 2 = 3`。位运算一次 `&` 就判断出"要不要处理这类动态",这就是 patchFlag 比"逐个属性比"快的根本。
...
📎 参考:[Vue 官方文档](https://cn.vuejs.org/)《Rendering Mechanism》、Vue runtime-core 源码