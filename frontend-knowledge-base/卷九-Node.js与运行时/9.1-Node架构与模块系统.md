...
**互操作(易踩坑):** ESM `import` CJS 时 `module.exports` 变 `default`;CJS 只能异步 `import()` 加载 ESM。新项目统一 ESM。
...
CJS(require/拷贝)vs ESM(import/静态);互操作易踩坑
```
...
能但有限制:ESM 可 import CJS(`module.exports` 变 default);CJS 只能动态 `import()` 加载 ESM,不能同步 require。混用易踩坑,建议统一。
