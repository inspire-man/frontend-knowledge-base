...
`Symbol` 生成**独一无二**的值,常做对象键,避免键名冲突:
...
const obj = { [s1]: "甲" };
obj[s1];                    // "甲"
Object.keys(obj);           // [] —— Symbol 键不可被 for...in / Object.keys 枚举
```

**全局 Symbol 注册表**(`Symbol.for` / `Symbol.keyFor`):

```js
const g1 = Symbol.for("app.id");
const g2 = Symbol.for("app.id");
g1 === g2;                  // true —— Symbol.for 复用全局注册表里的同一个
Symbol.keyFor(g1);          // "app.id" —— 反查 key
```

**内置 Symbol**:语言内部行为钩子,如 `Symbol.iterator` 定义迭代行为(1.6)、`Symbol.toPrimitive` 定义转原始值规则(第八节)。

> 用途:① 约定私有键;② 避免第三方库/代码的键冲突;③ 通过内置 Symbol 定义语言级行为。
...
`&&` 和 `||` **不返回布尔,返回决定结果的那个操作数**:

```js
0 || "默认值";      // "默认值" —— 0 是 falsy,返回右边
1 && "结果";        // "结果" —— 1 是 truthy,继续返回右边
null ?? "兜底";     // "兜底" —— ?? 只在 null/undefined 时兜底
0 ?? "兜底";        // 0 —— ?? 不管 falsy,只看 null/undefined
```

| 运算符 | 语义 | 返回 |
|--------|------|------|
| `a && b` | a 为 falsy 则返回 a,否则返回 b | 短路:遇 falsy 停 |
| `a \|\| b` | a 为 truthy 则返回 a,否则返回 b | 短路:遇 truthy 停 |
| `a ?? b` | a 为 null/undefined 则返回 b,否则返回 a | 只认 null/undefined |

> `??` 与 `||` 的区别:`||` 把 `0`/`""`/`false` 都当 falsy,`??` 只把 `null`/`undefined` 当空。要"给默认值但保留 0"就用 `??`。

### 比较运算符的隐式转换

```js
"2" > "10";   // true —— 两边都是字符串时按字典序(字符编码)比较,"2" > "1"
2 > "10";     // false —— 一边是数字,字符串转数字比较
"a" < 3;      // false —— "a" 转 NaN,NaN 参与比较恒 false
```

规则:两边都是字符串 → 字典序比较;否则转数字比较;转出 `NaN` → 比较结果恒 false。
...
**Q6:`Symbol.for` 和 `Symbol()` 有什么区别?**

`Symbol()` 每次调用都生成**全新的唯一值**(即使描述相同也不相等);`Symbol.for(key)` 会先查**全局注册表**,有就复用、没有则创建并登记,所以相同 key 返回**同一个** Symbol。用 `Symbol.keyFor` 可反查注册表里的 key(只对 `Symbol.for` 创建的有效)。

**Q7:为什么 `0.1 + 0.2 !== 0.3`?如何正确处理小数?**

十进制小数转二进制多是无限循环,64 位截断产生误差。处理:① 比较用 `Math.abs(a-b) < Number.EPSILON`;② 金额放大为分(整数运算);③ 高精度场景用 decimal 库或 `BigInt`。

**Q8:为什么 Symbol 作为对象键,不会被 `Object.keys` / `for...in` 枚举?**

Symbol 键的默认属性特性是 `enumerable: false`,且语言设计上把 Symbol 键与字符串键分开处理。要用 `Object.getOwnPropertySymbols()` 才能拿到。这也是它适合做"半私有"键的原因。

**Q9:说明 `??`(空值合并)与 `||` 的区别,各自适用场景。**

`||` 把**所有 falsy**(0/""/false/null/undefined)当假,用于"给默认值但 0 也有意义"会误伤;`??` 只把 `null`/`undefined` 当空,保留 0/""。所以"默认值但 0 合法"用 `??`,如 `count ?? 0`。

**Q10:为什么说 JS 的参数传递是"值传递"而不是"引用传递"?**

传参时复制栈里的值:基本类型复制值本身(改不影响外部);引用类型复制的是**地址的副本**(不是 C++ 那种"引用别名")。所以能改堆里的属性内容、但**重新赋值只改局部指向**、不影响外部的引用变量。严格说 JS 是"共享引用 + 值传递"。

---

📎 参考:ECMA-262(类型系统 / ToPrimitive)、[MDN《JavaScript 数据类型和数据结构》](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Data_structures)、[MDN《Symbol》](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Symbol)、V8《Memory management》/《escape analysis》