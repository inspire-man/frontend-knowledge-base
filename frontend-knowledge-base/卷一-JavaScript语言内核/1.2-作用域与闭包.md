...
> `const` 的"不可变"只约束**绑定**(不能重新赋值),不约束**值本身**:`const obj = {}; obj.x = 1` 合法(改的是对象内容,不是重新赋值)。
...
var bar = function () { console.log("bar"); };   // 函数表达式只提升 var,函数体留到赋值
```

> 关键区别:**函数声明**整体提升(含函数体),声明前可调用;**函数表达式**只提升变量名(值为 undefined),调用会报错。
...
counter.increment();   // 1
counter.get();         // 1
counter.count;         // undefined —— 真正私有
```

> IIFE(立即调用函数表达式,Immediately Invoked Function Expression)创建独立作用域,返回对象只暴露方法、隐藏 `count`——这是 ES6 模块出现前的主流模块化方案。
...
**Q7:`var`、`let`、`const` 的区别?`const` 真的"不可变"吗?**

区别:作用域(var 函数级,let/const 块级)、提升(var 提升为 undefined,let/const 有 TDZ)、重复声明、是否挂 window。`const` 只约束**绑定**不可重新赋值,不约束值本身:`const obj = {}; obj.x = 1` 合法。要真正冻结对象用 `Object.freeze`。

**Q8:函数声明和函数表达式的提升有何不同?**

函数声明**整体提升**(含函数体),声明前可调用;函数表达式只提升**变量名**(初始化为 undefined),函数体留到赋值执行。所以 `foo()`(声明)能调、`bar()`(表达式)调则报 TypeError。

**Q9:怎么用闭包模拟"私有变量"?**

用 IIFE 创建一次性的私有作用域,内部变量不暴露,只 `return` 出能访问它的方法(模块模式)。外部只能通过方法操作,拿不到内部变量本身——就是数据私有化。

**Q10:为什么闭包访问的是"定义时"的变量,而不是"调用时"的?**

因为 JavaScript 是**词法作用域(静态作用域)**:变量的可访问范围在**代码书写位置**确定,不随函数在哪调用而变。闭包通过内部槽 `[[Environment]]` 锁死"定义处的词法环境",所以永远拿到定义处那一份变量,不是调用处的。
