...
> 关键:预检是浏览器**自动发**的 OPTIONS,问“实际请求能用 PUT + content-type 吗?”;服务器回了允许头,浏览器才放行真实 PUT 请求。
...
**Q7:CORS 预检里 `Access-Control-Request-Method` 是干什么的?**

浏览器在 OPTIONS 预检里告诉服务器“**我即将用的 HTTP 方法**”(如 PUT)。服务器据此决定是否放行,并在 `Access-Control-Allow-Methods` 里回授。这避免了直接发不被允许的方法请求。

**Q8:同源策略为什么还要防“iframe 读取”?举一个具体例子?**

同源策略隔离的不只是 AJAX,还有 **DOM 读取**:恶意页面 `<iframe>` 嵌入银行页,若不受限就能读银行页的表单/登录态,盗取信息。同源策略禁跨域读 `window.parent`/`iframe.contentWindow`,只有 `postMessage`(带 origin 校验)才能安全通信。

---

📎 参考:[MDN《CORS》](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/CORS)《同源策略》、W3C Fetch 规范