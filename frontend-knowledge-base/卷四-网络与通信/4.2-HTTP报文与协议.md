...
或 **DevTools → Network → 点开任意请求 → Headers 面板**,看"Request Headers"(浏览器发的)和"Response Headers"(服务器回的)——这正是上面四段结构的实际形态。

> 实操:用 `curl -v` 发个请求,对照输出里的起始行/头部/空行/主体,比背概念更直观。
...
📎 参考:[MDN《HTTP 消息》](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Messages)《HTTP 状态码》、RFC 9110