...
| 指令 | 含义 |
|------|------|
| `max-age=N` | 缓存 N 秒(相对时间) |
| `s-maxage=N` | 同 max-age,但只对**共享缓存(CDN/代理)**生效 |
| `no-store` | 完全不存 |
| `no-cache` | 可存,但**每次用前先协商**(304) |
| `private` | 只能浏览器缓存,不缓存到 CDN |
| `public` | 可被 CDN 等公共缓存 |
| `must-revalidate` | 过期后必须回源验证,不能用过期缓存 |
| `immutable` | 内容永不变,不重新验证 |
| `stale-while-revalidate=N` | 过期后先用旧缓存,同时后台刷新 |

**易错:** `no-store`(不存)vs `no-cache`(存但每次验);`max-age`(所有缓存)vs `s-maxage`(仅 CDN)。

**Expires(历史,已被 Cache-Control 取代):**

```http
Expires: Wed, 21 Oct 2026 07:28:00 GMT   ; 绝对时间,受客户端时钟偏差影响
```

> `Cache-Control: max-age` 是**相对时间**(从请求起算),优于 `Expires` 的**绝对时间**(客户端时钟不准会出错)。两者都存在时 `Cache-Control` 优先。
...
**Q7:`s-maxage` 和 `max-age` 有什么区别?**

`max-age` 对**所有缓存**生效(浏览器 + CDN);`s-maxage` **只对共享缓存(CDN/代理)**生效,且优先级高于 `max-age`。所以可"浏览器缓存短、CDN 缓存长"分层控制。

**Q8:`must-revalidate` 有什么作用?**

强制**过期后必须回源验证**(返回 304 才能继续用),不允许在断网等情况下使用过期缓存。适用于"数据一致性要求高"的资源,防止拿到过期内容。

---

📎 参考:[MDN《HTTP 缓存》](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Caching)、RFC 9111、Google Web Fundamentals《HTTP 缓存》