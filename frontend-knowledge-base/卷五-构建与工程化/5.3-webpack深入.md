...
**把"人喜欢的代码"(模块化/预编译/语义化)转成"机器喜欢的代码"(少量文件/兼容/无注释/短变量)**。深挖要懂 **loader/plugin 的机制区别** 与 **tapable 事件体系**。
...
HMR 比"整体刷新"更进一步:**只替换改动的模块,状态不丢**。底层三步:

1. **WebSocket 长连接**:dev server 与浏览器保持连接,文件变化时**主动推送**更新
2. **模块热替换**:浏览器收到通知后,用 HMR runtime **只替换受影响模块**(而非整页刷新)
3. **状态保留**:配合框架 API(如 Vue 的 `import.meta.hot.accept`)替换模块时**保留组件状态**
...
> **先测量(`--profile`/speed-measure)再优化;先提速再减积**。
...
内容不变 hash 不变 → 命中强缓存;内容变 hash 变 → 新文件名 → 浏览器拿新文件。解决"旧文件被缓存、用户看不到更新"的老大难(呼应 [4.3](/卷四-网络与通信/4.3-HTTP缓存与协商))。
...
📎 参考:[webpack 官方文档](https://webpack.js.org/)《Concepts》《Guides》、tapable、speed-measure-webpack-plugin