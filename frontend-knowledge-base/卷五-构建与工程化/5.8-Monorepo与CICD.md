...
**Q7:qiankun 的 JS 沙箱有哪几种?各自特点?**

① **ProxySandbox(默认)**:Proxy 拦截 window 读写,对每个子应用造一个独立 `fakeWindow`,支持多子应用并存、互不污染;② **SnapshotSandbox(快照)**:激活存快照、失活对比恢复,兼容 IE11 但性能差、不支持多实例;③ **LegacySandbox**:Proxy 改主应用 window,仅单例,已少用。

**Q8:微前端适合什么场景?什么时候不该用?**

✅ 适合:门户型产品(多独立产品共享登录/菜单)、渐进式重构大应用、多技术栈并存、需独立部署/灰度。❌ 不该用:中小单体、团队协同顺畅、无独立部署需求——额外通信/沙箱复杂度大于收益,别为拆分而拆分。

---

📎 参考:pnpm Workspace 文档、Turborepo 文档、GitHub Actions 文档、[qiankun](https://qiankun.umijs.org/)、[Module Federation](https://webpack.js.org/concepts/module-federation/)