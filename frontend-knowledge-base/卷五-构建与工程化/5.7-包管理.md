...
**三种依赖位置:`dependencies` / `devDependencies` / `peerDependencies`**

| | dependencies | devDependencies | peerDependencies |
|---|-------------|----------------|------------------|
| 含义 | 运行时依赖 | 开发/构建时依赖 | 要求**宿主**提供的依赖 |
| 消费者安装 | ✅ 自动装 | ❌ 不装 | ❌ 要求宿主自己装且版本匹配 |
...
> 关键:发布到 npm 后,消费者只会**自动安装 `dependencies`**;`devDependencies` 不装;`peerDependencies` 是"声明依赖由宿主提供"(插件常用,如 React 插件声明 `peer react`),版本不匹配会报 peer 冲突。
...
📎 参考:pnpm《Motivation》《Workspace》、npm《Semver》、[Bun 文档](https://bun.sh/docs)