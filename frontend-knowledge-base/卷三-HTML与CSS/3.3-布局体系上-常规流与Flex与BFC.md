...
overflow: hidden / auto;   /* 副作用:裁剪溢出内容 */
position: absolute / fixed;
float: left / right;
display: flex / grid;      /* 本身建立一个新的 FC */
```
...
| `align-content` | **多行**交叉轴分布 | `stretch`/`center`/`space-between` |
| `gap` | 项目间距 | `12px` / `12px 8px`(行 列) |

**易混:`align-items` vs `align-content`**

| | 作用 | 生效前提 |
|---|------|---------|
| `align-items` | 每行内项目在**交叉轴**的对齐 | 单行(或每行内部) |
| `align-content` | **多行整体**在交叉轴的分布 | `flex-wrap: wrap` 且多行 |

> 单行时 `align-content` 无效果(只有一行,无所谓"多行分布");多行时两者都发挥作用(一个管行内、一个管行与行的间距)。
...
Flex:容器(flex-direction/justify/align-items/wrap/align-content/gap)
     + 项目(grow/shrink/basis);收缩 ∝ shrink×basis;align-items 单行,align-content 多行
```
...
收缩量 ∝ `flex-shrink × flex-basis`。基准尺寸大(内容长)的项目,在空间不足时按比例**缩得更多**。这就是 flex 收缩算法的直接结果。

**Q5:为什么说"清浮动"的本质是 BFC 而非 clear 属性?**

`clear: both` 是"强制元素避开浮动"(往下排),但不解决父容器高度塌陷;BFC 通过"独立隔离 + 包裹内部浮动"直接**算入浮动高度**,治本。二者常配合。

**Q6:inline 元素为什么对 height/margin-top 不敏感,而 inline-block 可以?**

`inline` 属于**行内格式化上下文(IFC)**,盒模型语义弱(高度由行高决定、垂直 margin 不生效);`inline-block` 内部是"块级盒模型"、外部"行内排布",所以能设宽高和垂直 margin。

**Q7:`align-items` 和 `align-content` 的区别?**

`align-items` 管**每行内项目**在交叉轴的对齐(单行即生效);`align-content` 管**多行整体**在交叉轴的分布(仅 `flex-wrap: wrap` 多行时生效)。单行时 align-content 无效。

**Q8:`justify-content` 的 `space-between` 和 `space-around`、`space-evenly` 区别?**

`space-between` 两端贴边、中间均分(首尾元素贴容器边缘);`space-around` 每个元素两侧留相等空间(两端各一半);`space-evenly` 所有间隙完全相等(含两端)。间距均匀度:evenly > around > between。

---

📎 参考:[MDN《BFC/格式化上下文》](https://developer.mozilla.org/zh-CN/docs/Web/CSS/CSS_flow_layout/Introduction_to_formatting_contexts)、[MDN《Flexbox》](https://developer.mozilla.org/zh-CN/docs/Web/CSS/CSS_flexible_box_layout)、[MDN《外边距折叠》](https://developer.mozilla.org/zh-CN/docs/Web/CSS/CSS_box_model/Mastering_margin_collapsing)