...
**Q4:`box-sizing: content-box` 和 `border-box` 的区别?工程上为什么统一 `border-box`?**

`content-box` 的 `width` 只指内容区,实际占宽 = width + padding + border(易溢出);`border-box` 的 `width` 已含 padding + border,整体尺寸直观可算。工程统一 `* { box-sizing: border-box }`,因为设 `width: 100%` 时不会因 padding/border 溢出父容器。

**Q5:为什么说"包含块"≠"父元素"?举一个反例?**

大多数时侯两者重合,但有例外:① `absolute` 元素的包含块是**最近的非 static 定位祖先**,不一定是直接父元素;② `fixed` 元素默认是视口;③ 有 `transform` 等的祖先会劫持 fixed/absolute 的包含块。所以是"按定位规则算出的参照物",不是简单的父元素。

---

📎 参考:[MDN《Containing block》](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Containing_block)、CSS2.1 规范 §10.1、CSS Transforms 规范(containing block 章节)