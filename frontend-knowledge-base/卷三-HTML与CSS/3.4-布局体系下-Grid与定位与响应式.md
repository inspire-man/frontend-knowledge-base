...
> 用字符串画出网格结构(一行一个字符串、列用空格分隔),再给子项指定 `grid-area`。比手写 `grid-column/row` 更直观,尤其适合"页头/侧栏/内容/页脚"这类整体布局。
...
**Q7:Grid 的 `grid-template-areas` 有什么用?**

用字符串画出网格结构(一行一个字符串、空格分隔列),再给子项指定 `grid-area` 名,直观描述"页头/侧栏/内容/页脚"式整体布局,比手写 `grid-column/row` 可读性高。

**Q8:什么会创建"层叠上下文"?为什么它让 z-index 失效?**

会创建:定位元素 + z-index 非 auto、`transform`/`filter`/`opacity<1`/`will-change`、`fixed`/`sticky`。因为**不同层叠上下文的 z-index 不跨上下文比较**——子元素 z-index 再大,也被父级上下文"锁"在里面,出不了父层。
