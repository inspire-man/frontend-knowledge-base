...
> 记忆:`grid-template` 定"你画好的格",`grid-auto` 定"多出来的格用多大"。
...
`transform` 会让祖先成为**新的包含块**(见 3.2),于是 `fixed` 不再相对视口,而相对这个祖先定位,视觉上"固定失效/跑偏"。这是面试高频组合陷阱。

---

📎 参考:MDN《CSS Grid》《position》《容器查询》、Web.dev《响应式设计》、CSS Grid 规范